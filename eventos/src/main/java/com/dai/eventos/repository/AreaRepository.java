...
public interface AreaRepository extends JpaRepository<Area, Long> {
    ...
    @Query(value = "select distinct area from Area area left join fetch area.camaras",
        countQuery = "select count(distinct area) from Area area")
    Page<Area> findAllWithEagerRelationships(Pageable pageable);

    @Query("select distinct area from Area area left join fetch area.camaras")
    List<Area> findAllWithEagerRelationships();

    @Query("select area from Area area left join fetch area.camaras where area.id =:id")
